...
if ( ! class_exists( 'AviaHelper' ) ) 
{
	...
	class AviaHelper
	{
		 ...
		 * @param string $attr_name					e.g. 'margin', 'padding' ....
		...
		static public function multi_value_result_lockable( $multi_value, $attr_name, $directions = array( 'top', 'right', 'bottom', 'left' ) )
		{
			...
			$css_rules = ! empty( $css_rules_val ) ? $attr_name . ': ' . implode( ' ', $css_rules_val ) . ';' : '';
			$css_rules_with_0 = ! empty( $css_rules_val ) ? $css_rules : $attr_name . ': 0;' ;
			 ...
			 * 'locked_opt_info':		option values as entered in array to show as locked option values ( 1 entry -> copied to others )
			 */
			$result = array( 
		 ...
		 * @return string
		 */
		static public function get_url( $link, $post_id = false ) 
		{
			$link = explode( ',', $link, 2 );
...
//    					'\.+$'					=> '', 
		 ...
		 * 
		 * @since < 4.0
		 * @since 4.8				added $default - needed for element template overrides
		 * @param array $atts
		 * @param array $default
		 * @return array
		 */
