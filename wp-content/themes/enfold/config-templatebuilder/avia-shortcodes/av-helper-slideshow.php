...
if ( ! class_exists( 'avia_slideshow' ) )
{
	class avia_slideshow
	{
		...
		public function __construct( array $config )
		{
			...
			$this->config = array_merge( avia_slideshow::default_args(), $config );
		...
		/**
		 * Renders the slides. use when we did use sub-shorcodes to define the images
		 *
		 * @return string
		 */
		...
		protected function slide_navigation_arrows()
		{
			...
			$html .= 	"<a href='#prev' tabindex='-1' class='prev-slide' " . av_icon_string( 'prev_big' ) . ">" . __( 'Previous', 'avia_framework' ) . "</a>";
			$html .= 	"<a href='#next' tabindex='-1' class='next-slide' " . av_icon_string( 'next_big' ) . ">" . __( 'Next', 'avia_framework' ) . "</a>";
			$html .= '</div>';
